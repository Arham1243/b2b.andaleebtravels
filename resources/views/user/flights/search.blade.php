                    ...
                    <div class="flight-card__main">
                        ...
                        <div class="flight-info__airline">
                            <div class="airline-logo">
                                <img src="https://img.logo.dev/{{ $firstSeg['carrier'] }}.png?token=YOUR_TOKEN" onerror="this.src='https://ui-avatars.com/api/?name={{ $firstSeg['carrier'] }}&background=random'" alt="Carrier">
                            </div>
                            <span class="airline-name">{{ $firstSeg['carrier_name'] ?? $firstSeg['carrier'] }}</span>
                        </div>
                        ...
                        <div class="flight-info__action">
                            ...
                            <a href="#details-{{ $loop->index }}" class="details-toggle" data-bs-toggle="collapse">
                                ...
                                @foreach ($leg['segments'] as $seg)
                                    <div class="segment-row">
                                        ...
                                        <div class="seg-divider">
                                            ...
                                            <span class="flight-no">{{ $seg['carrier'] }}{{ $seg['flight_number'] }}</span>
                                        </div>
...
.connector { height: 2px; flex: 1; background: #cbd5e1; }
.dot { width: 8px; height: 8px; border-radius: 50%; background: #cbd5e1; }
.plane-icon { padding: 0 10px; color: var(--primary); font-size: 1.2rem; }

.path-duration { font-size: 0.75rem; color: var(--text-muted); }
.path-stops { font-size: 0.75rem; font-weight: 600; margin-top: 4px; }
.path-stops.non-stop { color: #10b981; }
.path-stops.has-stops { color: #f59e0b; }
...
.price-tag .currency { font-size: 0.9rem; font-weight: 600; color: var(--text-muted); }
.price-tag .amount { font-size: 1.5rem; font-weight: 800; color: var(--primary); }
...
/* Collapsible Area */
.flight-card__details { background: #f8fafc; border-top: 1px solid var(--border); }
...
.seg-divider { display: flex; flex-direction: column; align-items: center; color: var(--text-muted); font-size: 0.75rem; }

/* Responsive */
@media (max-width: 768px) {
    .flight-card__main { flex-direction: column; gap: 1.5rem; }
    .flight-info__action { border-left: none; padding-left: 0; border-top: 1px solid var(--border); padding-top: 1.5rem; width: 100%; align-items: center; }
}
    </style>
@endpush

...
<head>
    ...
    <title>{{ $forAdmin ? 'Admin: ' : '' }}Hotel cancelled - {{ $booking->booking_number }}</title>
...
<body>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
            <tr>
                <td class="banner-admin">
                    Internal notice: a hotel booking was cancelled on the portal.
                    @if($cancelType)
    ...
    @if (!$forAdmin)
        <p class="muted" style="margin-bottom:12px;">Dear {{ $booking->lead_full_name }},</p>
        <p class="muted">Your hotel reservation below has been cancelled with the supplier. If a refund applies, it will follow your payment provider's timeline.</p>
    @else
        <p class="muted">Cancellation recorded for the booking below.</p>
    @endif

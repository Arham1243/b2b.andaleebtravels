...
$passengersData = [
    ...
    'passengers' => [['type' => 'ADT', 'title' => 'Mr', 'first_name' => 'MIN', 'last_name' => 'MOD', 'dob' => '1990-05-05', 'nationality' => 'PK', 'issuing_country' => 'PK', 'passport_no' => 'AB1234567', 'passport_exp' => '2030-01-01']],
];
...
$version = $vm[1] ?? '0';
echo "hold ok=" . var_export($hold['success'] ?? null, true) . " universal={$universal} air={$airLoc} version={$version}\n";
echo "retrieve ok=" . var_export($retrieve['success'] ?? null, true) . "\n";

// Minimal UR modify - add OSI remark via AirAdd (if supported) or test empty modify structure
        ...
        <universal:UniversalRecordModifyReq xmlns:universal="{$uniNs}" xmlns:com="{$comNs}" TraceId="trace_min" AuthorizedBy="Zeeshan" TargetBranch="P7250866" Version="{$version}" ReturnRecord="true">
            ...
            <universal:UniversalModifyCmd Key="mod1">
                <universal:AirAdd ReservationLocatorCode="{$airLoc}">
                    <universal:OSI Text="TEST OSI FROM API"/>
                </universal:AirAdd>
...
curl_setopt_array($ch, [
    ...
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => $minimal,
    CURLOPT_TIMEOUT => 90,
    CURLOPT_HTTPHEADER => ['Content-Type: text/xml;charset=UTF-8', 'Authorization: Basic ' . $creds],
]);
$response = curl_exec($ch);
$http = (int) curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);
echo "minimal modify http={$http}\n";
